...
$( document ).ready(function() {  
  ...
  $.ajax({ url:'http://api.openweathermap.org/data/2.5/weather?q=seoul&APPID=493227ed81dda9925c44ec2f77ae948a&units=metric', dataType:'json', type:'GET', success:function(data){ var $Icon = (data.weather[0].icon).substr(0,2); var $Temp = Math.floor(data.main.temp) + 'º'; var $city = data.name; $('.CurrIcon').append('<small><i class="' + weatherIcon[$Icon] +'"></i></small>');$('.CurrText').append('<small>' + weatherText[$Icon] +'</small>'); $('.CurrTemp').prepend('<small>'+$Temp+'</small>'); $('.City').append('<small>'+$city+'<small>'); } })
});
